locals {
    ...
    # Custom data script for Ubuntu 24.04 setup
    default_custom_data = var.custom_data != null ? var.custom_data : base64encode(templatefile("${path.module}/scripts/ubuntu-setup.sh", {
      timezone = var.timezone
      admin_username = var.admin_username
      enable_syslog_server = var.enable_syslog_server
      syslog_port = var.syslog_port
      syslog_protocol = var.syslog_protocol
      syslog_allowed_networks = var.syslog_allowed_networks
      syslog_log_retention_days = var.syslog_log_retention_days
      syslog_max_log_size = var.syslog_max_log_size
      enable_syslog_tls = var.enable_syslog_tls
      syslog_custom_rules = var.syslog_custom_rules
    }))
}
